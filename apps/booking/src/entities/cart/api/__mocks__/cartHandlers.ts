...
export const cartHandlers = [
  http.get(`${env.VITE_API_ENDPOINT}/cart`, async ({ request }) => {
    try {
      ...
      return HttpResponse.json(mockCartDto(maybeCart, products), {
        status: 200,
      });
    } catch {
  ...
  http.patch<object, { version: number; items: CartItemDto[] }>(
    `${env.VITE_API_ENDPOINT}/cart`,
    async ({ request }) => {
      try {
        const { userId } = await verifyAccessToken(
          parseTokenFromRequest(request),
